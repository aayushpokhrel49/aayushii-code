    ...
    impl Component for MessageNotification {
        ...
        fn preview(_window: &mut Window, cx: &mut App) -> AnyElement {
            ...
            impl WorkspaceError for PortalSetupError {
                ...
                fn secondary_message(&self) -> Option<SharedString> {
                    Some("AAYKRA needs an xdg-desktop-portal implementation to open files.".into())
                }
                fn severity(&self) -> ErrorSeverity {
                    ErrorSeverity::Critical
                }
                fn primary_action(&self) -> ErrorAction {
                    ErrorAction::link(
                        "See Docs",
                        "https://code.aayushpokhrel.info.np/docs/linux#i-cant-open-any-files",
                    )
                }
            }
            ...
            impl WorkspaceError for UpdateRequiredError {
                fn primary_message(&self) -> SharedString {
                    "An update is required to continue using AAYKRA AI.".into()
                }
                fn severity(&self) -> ErrorSeverity {
                    ErrorSeverity::Critical
                }
                fn primary_action(&self) -> ErrorAction {
                    ErrorAction::link("Update AAYKRA", "https://zed.dev/releases")
                }
