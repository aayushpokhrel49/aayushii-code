...
pub fn prevent_root_execution() {
    ...
    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running AAYKRA as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of AAYKRA.
...
pub fn get_zed_cli_path() -> Result<PathBuf> {
    ...
    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        ...
        // bin/aaykra.exe is for installed builds, ./cli.exe is for development builds.
        &["bin/aaykra.exe", "./cli.exe"]
