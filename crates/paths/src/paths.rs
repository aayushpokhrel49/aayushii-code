...
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".aaykra_server").unwrap());
    *CACHED
}

// Remove this once 223 goes stable
/// Returns the relative path to the `.aaykra_wsl_server` directory on the wsl host.
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".aaykra_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the relative path to a `.aaykra` folder within a project.
pub fn local_settings_folder_name() -> &'static str {
    ".aaykra"
}

/// Returns the relative path to a `.aayushicode` folder within a project, used as a
/// fallback when there is no `.aaykra` folder.
...
/// Picks the `.aaykra` config file over its `.aayushicode` counterpart. The
/// `.aayushicode` file is only used when it exists and the `.aaykra` one does not.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".aaykra/settings.json").unwrap());
    *CACHED
}

/// Returns the relative path to a `.aayushicode/settings.json` file within a project, used as a
/// fallback when there is no `.aaykra/settings.json`.
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".aaykra/tasks.json").unwrap());
    *CACHED
}

/// Returns the relative path to a `.aayushicode/tasks.json` file within a project, used as a
/// fallback when there is no `.aaykra/tasks.json`.
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".aaykra/debug.json").unwrap());
    *CACHED
}

/// Returns the relative path to a `.aayushicode/debug.json` file within a project, used as a
/// fallback when there is no `.aaykra/debug.json`.
